...
4. Если нужно дебажить гем (этот шаг можно автоматизировать до одной команды + автоматически выполнять при 
    ...
    * прогоняем: `node script/package.js` чтобы сгенерировать результирующий проектный `package.json`
    * устанавливаем зависимости: `yarn install`
    * линкуем локальный гем в node modules: `yarn run linklocal`
...
админов. После получения доступа, необходимо авторизоваться: `npm adduser`.

Для того, чтобы гем появился в приватном [registry](registry.railsc.ru), необходимо собрать и опубликовать npm-пакет:

1. `yarn run build`   - сборка пакета вебпаком (необходимо убедиться, что в `package.json` есть нужная команда `build`)
2. `yarn publish` - публикация пакета в приватном registry.
